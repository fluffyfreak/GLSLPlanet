...
float GetHeight(in vec3 p)
{
	...
	continents = amplitude[0] * ridged_octavenoise(octaves[0], clamp(continents, 0.0, 0.6), lacunarity[0], p, 1.0, 1.0);
	float mountains = ridged_octavenoise(octaves[2], 0.5, lacunarity[2], p, 1.0, 1.0);
	float hills = octavenoise(octaves[2], 0.5, lacunarity[2], p, 1.0, 1.0) * amplitude[1] * river_octavenoise(octaves[1], 0.5, lacunarity[1], p, 1.0, 1.0);
	...
	if (n > 0.0) {
		...
		mountains = octavenoise(octaves[3], 0.5, lacunarity[3], p, 1.0, 1.0) * amplitude[2] * mountains*mountains*mountains;
		if (n < 0.4) 
			n += 2.0 * n * mountains;
		else 
			n += mountains * 0.8;
	...
	// craters
	n += 3.0*impact_crater_function(octaves[5], amplitude[5], frequency[5], lacunarity[5], p);
	n = maxHeight*n;
	n = ((n<0.0) ? 0.0 : n);
	n = ((n>1.0) ? (2.0-n) : n);
	return n;
