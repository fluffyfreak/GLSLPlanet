...
float GetHeight(in vec3 p)
{
	...
	float n = (river_function(octaves[9], amplitude[9], frequency[9], lacunarity[9], p, 0)*
		river_function(octaves[7], amplitude[7], frequency[7], lacunarity[7], p, 0)*
		river_function(octaves[6], amplitude[6], frequency[6], lacunarity[6], p, 0)*
	...
	if (n > 0.0) {
		...
		n += h*river_octavenoise(octaves[7],
			0.5*octavenoise(octaves[6], 0.5, lacunarity[6], p, 1.0, 1.0), lacunarity[7], p, 1.0, 1.0);
	}
	...
	if (n > 0.0) {
		...
		n += n*0.7*ridged_octavenoise(octaves[5],
			0.7*octavenoise(octaves[6], 0.6, lacunarity[6], p, 1.0, 1.0), lacunarity[5], p, 1.0, 1.0);

		//jagged surface for mountains
		if (n > 0.25) {
			n += (n-0.25)*0.1*octavenoise(octaves[3],
				clamp(h*0.0002*octavenoise(octaves[5], 0.6, lacunarity[5], p, 1.0, 1.0),
				 0.5*octavenoise(octaves[3], 0.5, lacunarity[3], p, 1.0, 1.0),
				 0.6*octavenoise(octaves[4], 0.6, lacunarity[4], p, 1.0, 1.0)), lacunarity[3], p, 1.0, 1.0);
		}
