	...
	//This is among the most complex of terrains, so I'll use this as an example:
{
	...
	if (n > 0.0) {
		...
		n += h*0.2*ridged_octavenoise(octaves[7], 0.5*octavenoise(octaves[6], 0.5, lacunarity[6], p), lacunarity[7], p);
		...
		n += n*0.7*ridged_octavenoise(octaves[5],
			0.5*octavenoise(octaves[6], 0.5, lacunarity[6], p), lacunarity[5], p);
		...
		if (n < 0.001){
			n += n*0.2*ridged_octavenoise(octaves[1],
				0.5*octavenoise(octaves[2], 0.5, lacunarity[2], p), lacunarity[1], p);
		} else if (n <0.01){
