...
uniform vec2 batch_offset;
uniform float batch_factor;

//uniform int p[256]; // does not work on fermi cards
int p[256] = { // does not work on earlier cards
...
void main(){
    vec2 uv = gl_TexCoord[0].xy*batch_factor+batch_offset;

    float value = noise((uv-0.5)*size+offset)*height;
