...
void main(){
    if(gl_FrontFacing){
        ...
        vec3 ac = texture2D(ambient_occlusion, uv).rgb;
        ...
        // atmospheric effects
        //vec3 atmosphere = vec3(51.0/255.0, 195.0/255.0, 255.0/255.0);
        if(fog){
            vec3 atmosphere = vec3(0.3, 0.7, 1.0);
            float falloff = 1.0-sqrt(depth) * 0.6;
            diffuse *= falloff;
            diffuse += atmosphere * clamp(pow(depth, 2.0) * 5.0, 0.0, 0.2);
        }
        
        gl_FragColor = vec4(pow(diffuse, vec3(gamma)), gl_Color.w);
    }
    else{
